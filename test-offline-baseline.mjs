...
// its `download`-config path. Rowbase never enables that config (it only calls
// Papa.parse(string) and Papa.unparse(...)), so this is dead code carried
// inside the preserved dependency. It is the only allowed XMLHttpRequest and is
// uniquely identifiable by papaparse's downloadConfig marker.
...
// papaparse remote-download path, uniquely marked by its `downloadConfig`.
  ...
  for (const re of forbidden) {
    ...
    while ((match = global.exec(working)) !== null) {
      if (re.source === "XMLHttpRequest" && allowXhr) {
        const fragment = working.slice(match.index, match.index + 200);
        if (fragment.includes(allowedXhrDownloadMarker)) {
          continue;
        }
      }
