...
Note: color-by and X are separate columns in the test above (Owner vs Category). Series names are pushed in appearance order (B then A, matching the first row each appears in), and `series` is built over `seriesNames` in that order — so `series[0]` is B, `series[1]` is A, and their `values` arrays align to `labels` order (outer map over `labels`, inner `aggregates.get(keyFor(label, color))`). If color-by is unset, `seriesOrder` is `[""]` and a single "value" series is returned.
